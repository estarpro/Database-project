...
$db_host = '182.254.138.123';
$db_port = 3306;
$db_user = 'project_test';
$db_pass = 'jUx|5pJl7H,HIhE,{b<&';
...
$op = $_POST['op'];
if ($op !== 'new' && $op !== 'login' && $op !== 'delete' && $op !== 'change')
...
// Mitigate timing attacks (probing for valid usernames)
    ...
    if ($hasher->CheckPassword($pass, $hash)) {
        $what = 1;  /*'Authentication succeeded';*/
        $_session['valid_user']=$user;
    } else {
        $what = 'Authentication failed';
        $op = 2;   /*'fail'; // Definitely not 'change'*/
        exit(0);
    }
...
$stmt->close();
$query="select role from useers where username='".$user."'";
$result=$db->query(query);
$role=$result->fetch_field();
if($role=='doctor')
{
    $url=doctor.html;
    header("location: $url");
